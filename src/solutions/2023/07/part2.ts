...
export const solution: Solution<number> = (input: string[]) => {
	...
	const parsedGames = games.map((game) => {
		...
		const partiallySortedChars = [...parsedChars.entries()].sort((a, b) => b[1] - a[1]);
		if (partiallySortedChars[0][1] === 5) return [game[0], game[1], getHandType(partiallySortedChars)] as const;

		const jokers = parsedChars.get('J') ?? 0;
		const best = partiallySortedChars[0][0] === 'J' ? partiallySortedChars[1] : partiallySortedChars[0];
		parsedChars.set(best[0], parsedChars.get(best[0])! + jokers);
		parsedChars.delete('J');
...
await benchmark(2_023, 7, 2, solution);
